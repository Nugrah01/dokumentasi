        ...
        <main class="page-body">
            ...
            <div class="page-banner">
                <div class="page-banner-left">
                    ...
                    <div>
                        <h2 class="page-banner-title">Edit Dokumentasi Pada Halaman Yang Di Tentukan</h2>
                        <p class="page-banner-sub">Ubah isi form dibawah untuk memperbarui dokumentasi, serta cantumkan halaman dan kategori.</p>
                    </div>
                </div>
            ...
            {{-- Form Card --}}
            <div class="form-card">
                <form id="form-hal-doc" method="POST" action="{{ route('halaman.update', $halaman->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="field-group">
                        ...
                        <select id="bagian-select" name="bagian_id" class="field-select" required>
                            ...
                            @foreach($bagian as $b)
                                <option value="{{ $b->id }}" {{ $b->id == ($halaman->kategori->bagian_id ?? null) ? 'selected' : '' }}>
                                    {{ $b->judul }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="field-group">
                        ...
                        <select id="kategori-select" name="kategori_id" class="field-select" required>
                            <option value="{{ $halaman->kategori_id }}">{{ $halaman->kategori->judul }}</option>
                        </select>
                    </div>
                    ...
                    <div class="field-group">
                        ...
                        <textarea
                            ...
                            required>{{ $halaman->deskripsi }}</textarea>
                    </div>

                    {{-- Jawaban --}}
                    <div class="field-group">
                        <label class="field-label">Jawaban</label>
                        <textarea id="editor" name="jawaban">{{ $halaman->jawaban }}</textarea>
                    </div>
                    ...
                    <div class="form-footer">
                        <div class="status-group">
                            ...
                            <select name="status" class="field-select field-select-sm">
                                ...
                                <option value="delete"  {{ $halaman->status == 'delete'  ? 'selected' : '' }}>Deleted</option>
                            </select>
                        </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (e) {
        ...
        try {
            ...
            if (response.ok) {
                pushNotifToStorage('Dokumentasi berhasil diperbarui', 'edit');
                window.location.href = "{{ route('halaman.index') }}";
            } else {
                submitBtn.disabled = false;
                submitBtn.style.opacity = '';
                submitBtn.style.cursor  = '';
                pushNotifToStorage('Gagal memperbarui dokumentasi', 'danger');
                if (data.errors) {
            ...
            submitBtn.style.opacity = '';
            submitBtn.style.cursor  = '';
            pushNotifToStorage('Terjadi kesalahan saat memperbarui data', 'danger');
        }
    });
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const currentKategoriId = "{{ $halaman->kategori_id }}";

    async function loadKategori(bagianId) {
        try {
            ...
            kategoriList.forEach(kategori => {
                ...
                if (kategori.id == currentKategoriId) option.selected = true;
    ...
    if (bagianSelect.value) loadKategori(bagianSelect.value);
    bagianSelect.addEventListener("change", function () { loadKategori(this.value); });
});
</script>
